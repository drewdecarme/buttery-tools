...
import type { ButteryTokensConfig } from "./getButteryTokensConfig";
...
export async function launchPlaygroundGetConfig(
  ...
  try {
    ...
    throw LOG.fatal(
      new Error(
        `Fatal error when trying to resolve a single tokens configuration from the '.buttery/tokens' config: ${
