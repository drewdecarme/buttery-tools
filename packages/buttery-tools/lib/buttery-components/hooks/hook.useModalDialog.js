 ...
 * );
 */
export const useModalDialog = (params) => {
    const modalRef = useRef(null);
    const [dialogState, setModalState] = useState();
    const { Portal, openPortal, closePortal } = usePortal();
    const dialogEventClickRef = useRef(null);
    const dialogEventCancelRef = useRef(null);
     ...
     */
    const dialogRef = useCallback((dialogNode) => {
        if (!dialogNode)
            return;
