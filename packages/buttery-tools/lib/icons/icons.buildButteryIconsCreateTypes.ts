...
export async function createTypes(dirs: ButteryIconsDirectories) {
  try {
    // get the icon files
    LOG_CLI.debug(`Fetching SVG files from: "${dirs.entry.svgDir}"...`);
    const iconFileNames = await readdir(dirs.entry.svgDir);
    LOG_CLI.debug(`Fetching SVG files from: "${dirs.entry.svgDir}"... done.`);

    // Create a types union
    LOG_CLI.debug("Creating types from icon file names...");
    ...
    LOG_CLI.debug("Creating types from icon file names... done.");
    ...
    LOG_CLI.debug(`Writing types to: "${typesFilePath}"...`);
    LOG_CLI.debug(`Types fileContent: ${fileContent}`);
    await writeFile(typesFilePath, fileContent);
    LOG_CLI.debug(`Writing types to: "${typesFilePath}"... done.`);
  } catch (error) {
