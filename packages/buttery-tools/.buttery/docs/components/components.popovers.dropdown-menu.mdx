...
title: DropdownMenu
meta:
  - title: DropdownMenu | Buttery Components
  - name: description
    content: A specific type of dropdown Popover component that makes it easy to add inline context menus that are 100% accessible and keyboard and mouse event aware.
---

# DropdownMenu

An component abstraction that that extends the [useDropdown](./hooks.useDropdown.mdx) hook and
incorpoates the [usePortal](./hooks.usePortal.mdx) hook to easily
...
{/* example:"lib/buttery-components/popovers/dropdown-menu/examples/DropdownMenu.example.tsx" */}
...
{/* example:"lib/buttery-components/popovers/dropdown-menu/examples/DropdownMenuBarebones.example.tsx" */}
...
{/* example:"lib/buttery-components/popovers/dropdown-menu/examples/DropdownMenuWithOffset.example.tsx" */}

### With Arrow

You can add an arrow between the target and the dropdown by adding a `dxArrow` option to the
`useDropdownMenu` hook.
{/* example:"lib/buttery-components/popovers/dropdown-menu/examples/DropdownMenuWithArrow.example.tsx" */}

### Positioning

You can position the element anywhere surrounding the target
{/* example:"lib/buttery-components/popovers/dropdown-menu/examples/DropdownMenuPositions.example.tsx" */}
...
{/* example:"lib/buttery-components/popovers/dropdown-menu/examples/DropdownMenuGroups.example.tsx" */}

### Nested Forms

It's not uncommon to open a modal where it's pupose is to collect information. In the case that the dialog
is a form, we need to ensure that any other nested forms are taken out of the context of the parent form. In
this case, surfacing a popover that adds another layer on the `topLevel` of the DOM is the best practice.

In doing so, we don't have to worry about `preventDefault` or `stopPropogation` since the forms work
independently of other, regardless if they're nested in the markup that you write.

#### Use case

In this use case, we're going to open a modal dialog that is a form. We're also going to add another form
in a typeahead to submit something that isn't associated with the parent form. You can think of this as
being required to fill out some information in a modal, but then also having an a option to add
an entry to something that you would then be able to select in the parent form.

{/* example:"lib/buttery-components/popovers/dropdown-menu/examples/DropdownMenuWithNestedForms.example.tsx" */}
